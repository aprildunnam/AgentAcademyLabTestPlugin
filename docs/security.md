...
| Session cookies / localStorage | `runtime/account/storage-state.json` (cleartext) | Filesystem permissions only | Until cache expiry or `clear` | Effectively equivalent to a logged-in session — a thief with this file can act as the workshop account |
...
- **The session-state file (`storage-state.json`).** Cookies and localStorage are stored in cleartext. A thief who steals `storage-state.json` can replay it in a browser and act as the workshop account *without* needing the password. This is a deliberate trade-off: the Playwright MCP doesn't expose a way to encrypt the storage state, and re-encrypting every page load would be impractical.
...
- **Backup software that copies user data en clair.** DPAPI-encrypted files travel encrypted, but `storage-state.json` and screenshots travel as-is. If your machine's backup includes `runtime/`, you're trusting your backup target.
...
- **Set tight filesystem permissions on `~/.claude/plugins/mcs-lab-auditor/runtime/`.** On a multi-user machine, ensure other accounts cannot read this directory. (DPAPI protects `credential.enc` either way, but `storage-state.json` and screenshots are not encrypted.)
